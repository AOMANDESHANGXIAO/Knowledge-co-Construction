...
type moveingCallbackType =
  | ((e: MouseEvent, arg: { left: number; top: number }) => void)
  | undefined;
...
export function useDivDrag({
  ...
  onMounted(() => {
    if (!dragBoxRef.value)
      return console.error("dragBoxRef 未绑定到需要移动的 dom 上！");
    dragBoxRef.value.addEventListener(
      "mousedown",
      mouseDownEventListenerHandle,
      false,
    );
  });
  onUnmounted(() => {
    dragBoxRef.value &&
      dragBoxRef.value.removeEventListener(
        "mousedown",
        mouseDownEventListenerHandle,
      );
  });
